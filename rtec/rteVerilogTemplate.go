...
//(Not recommended. Check the .sv EBMC file instead). To check this file using EBMC, run the following command:
//$ ebmc F_{{$block.Name}}.v{{range $polI, $pol := $block.Policies}} -p "{{$block.Name}}_policy_{{$pol.Name}}_state != {{if len $pol.States}}{{len $pol.States}}{{else}}1{{end}}"{{end}}
...
{{define "ebmcFunctionVerilog"}}{{$block := index .Functions .FunctionIndex}}{{$blocks := .Functions}}
//This file should be called embc_F_{{$block.Name}}.sv
//This is autogenerated code. Edit by hand at your peril!
