...
void print_data(uint32_t count, inputs_px_py_pz_pr_px2_t inputs, outputs_px_py_pz_pr_px2_t outputs) {
...
int main() {
    ...
    for (uint16_t run = 0; run < RUNS; run++) {
        ...
        start = clock();
        enforcervars_px_py_pz_pr_px2_t enf;
        inputs_px_py_pz_pr_px2_t inputs;
        outputs_px_py_pz_pr_px2_t outputs;
        
        px_py_pz_pr_px2_init_all_vars(&enf, &inputs, &outputs);
        ...
        while(count++ < TICKS_PER_RUN) {
            ...
            px_py_pz_pr_px2_run_via_enforcer(&enf, &inputs, &outputs);
...
void px_py_pz_pr_px2_run(inputs_px_py_pz_pr_px2_t *inputs, outputs_px_py_pz_pr_px2_t *outputs) {
    //do nothing
}
